...
if __name__ == "__main__":
    ...
    for method_name in all_methods:
        ...
        # --- 新增代码：跳过 'oxc_scheduler_noopt_releax' ---
        if method_name == 'oxc_scheduler_noopt_releax':
            print(f"  Skipping method as requested: {method_name}")
            continue
        # --- 新增代码结束 ---

        # 如果 method_labels 字典中没有这个键，则使用原始目录名作为标签
        label = method_labels.get(method_name, method_name)

        # 检查是否应该处理这个方法（例如，如果它在 labels 字典中，或者即使不在也要处理）
        # 在这个修改后的版本中，我们检查 method_name 本身，而不是依赖 labels 字典来决定是否跳过
        # （因为上面的 if 语句已经处理了明确要跳过的 relax 方法）
            ...
            if pd.notna(comp_time) and pd.notna(base_time):
                if base_time > 1e-9: # Avoid division by zero or near-zero
                    ...
                    valid_slowdown_count += 1
                     ...
                     slowdown[i] = np.nan # Base time is zero, comp time is positive -> NaN
                     # print(f"  Task {i}: Baseline time is zero, comparison time is {comp_time}. Setting slowdown to NaN.") # Reduce verbosity
                else:
                     slowdown[i] = 0.0 # Both times are zero/negligible
                     valid_slowdown_count += 1
